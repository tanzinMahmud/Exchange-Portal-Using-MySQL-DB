			...
			<ul>
			  ...
			  <?php echo " Welcome ADMIN, ".$_SESSION["admin_name"];?></a></li>
				...
				if(isset($_REQUEST["se_no"]) && strlen($_REQUEST["se_no"])>0 && is_numeric($_REQUEST["se_no"])){
					$sq="delete from customer where customer_id = ".$_REQUEST["se_no"];
					$a=updateSQL($sq);
					...
					<p align="center">Delete successful. Go to All customers</p><br>
					<form align="center" action="all_cus.php" method="post">

					<button class="oto_button" onclick="return check()">All customers</button>
					</form><?php
				}
		}
		else{
