	...
	<body>
		...
		<div>
			<ul>
			  ...
			  <?php echo " Welcome ADMIN, ".$_SESSION["name"];?></a></li>
			</ul>
			</div>
			</br>
			</br>
				...
				if(isset($_REQUEST["se_no"]) && strlen($_REQUEST["se_no"])>0 && is_numeric($_REQUEST["se_no"])){
					$sq="delete from old_product where product_id = ".$_REQUEST["se_no"];
					$a=updateSQL($sq);
					...
					<p align="center">Delete successful. Go to all sell to us order</p><br>
					<form align="center" action="sell_to_us.php" method="post">
